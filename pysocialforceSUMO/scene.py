...
class AgentState:
    """Tracks the state of pedestrians, e-scooters, bikes and other unmanaged vehicles
    (and social groups but this last one is deactivated)"""

    def __init__(self, state, groups, config, sumo_vehicles):
        self.default_tau = config("tau", 0.5)
        self.step_width = config("step_width", 0.4)  # delta time of a step
...
class EnvState:
    ...
    def obstacles(self, obstacles):
        """Input a list of shapely Points/tuples (start, end) as start and end of a line"""
        if obstacles is None:
            self._obstacles = []
        else:
            self._obstacles = []
            for start, end in obstacles:
                samples = int(np.linalg.norm((start[0] - end[0], start[1] - end[1])) * self.resolution)
                line = np.array(
                    list(
                        zip(np.linspace(start[0], end[0], samples), np.linspace(start[1], end[1], samples))
                    )
                )
